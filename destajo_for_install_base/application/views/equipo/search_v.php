<?php echo form_open('equipo/show_content', array('id'=>'form-search')); 
    // Obtener las busquedas almacenadas en session
    $b_campo = $this->session->userdata('buscar_campo_equipo');
    $b_criterio = $this->session->userdata('buscar_criterio_equipo');
    $b_texto = $this->session->userdata('buscar_texto_equipo');
?>
    			...
    			<tr>
    				...
    				<td>
                        <select class="select2" name="buscar_campo[]" data-placeholder="Columna">
                            <option value="numero_operacional" <?php if($value=='numero_operacional'): ?> selected="selected" <?php endif; ?>>N&uacute;mero operacional</option>
                        </select>
                    </td>
