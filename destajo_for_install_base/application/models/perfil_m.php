...
class Perfil_m extends CI_Model {
	 ...
	 */
	 ...
	 public function list_all($limit='', $offset='')
	 {
         ...
         if (!$ofield OR !$otype) {
             $ofield = $this->session->userdata('ofield_perfil') ? $this->session->userdata('ofield_perfil') : "perfil";
             $otype = $this->session->userdata('otype_perfil') ? $this->session->userdata('otype_perfil') : "asc";
         }
         ...
         $query['num_rows_wo_limit'] = $this->db->get('perfil')->num_rows();
         if ($limit!='' OR $offset!='') $this->db->limit($limit, $offset);
        
         // Obtener registros
         //-----------------------------
         $query['rows'] = $this->db->get('perfil');
         $query['num_rows'] = $query['rows']->num_rows();
         $this->db->flush_cache();
        
         // Almacenar ordenar en session
         //-----------------------------
         $this->session->set_userdata('order_field_perfil', $ofield);
         $this->session->set_userdata('order_type_perfil', $otype);
    ...
    public function create_search()
    {
        ...
        if (! $buscar_btn_avanzado) {
            if (! $campo OR count($campo) <= 0) {
                $campo = $this->session->userdata('buscar_campo_perfil');
                $criterio = $this->session->userdata('buscar_criterio_perfil');
                $texto = $this->session->userdata('buscar_texto_perfil');
            }
        }
        ...
        if ($buscar_btn_avanzado AND $campo) {
            if ($this->input->post('buscar_campo')) $this->session->set_userdata('buscar_campo_perfil', $this->input->post('buscar_campo'));
            if ($this->input->post('buscar_criterio')) $this->session->set_userdata('buscar_criterio_perfil', $this->input->post('buscar_criterio'));
            if ($this->input->post('buscar_texto')) $this->session->set_userdata('buscar_texto_perfil', $this->input->post('buscar_texto'));
        }
        
        // Se mando a buscar y no hay texto en la busqueda ni en session
        // so, eliminar de session
        if ($buscar_btn_avanzado AND ! $campo) {
            $this->session->unset_userdata('buscar_campo_perfil');
            $this->session->unset_userdata('buscar_criterio_perfil');
            $this->session->unset_userdata('buscar_texto_perfil');
        }    
    ...
    public function agregar()
    {
        $this->db->trans_start();
            ...
            // Agregar perfil
            $this->db->set('perfil', $this->input->post("perfil"));
            $this->db->set('descripcion', $this->input->post("descripcion"));
            $this->db->set('no_eliminar', $this->input->post("no_eliminar"));
            $this->db->insert('perfil');
            
            // Obtener id del perfil agregado
            $perfil_id = $this->db->insert_id();        
            
            // Agregar la relacion perfil permiso
            $permisos = $this->input->post('permisos');
            if (isset($permisos)) {
                foreach ($permisos as $key => $value) {
                    $this->db->set('fk_perfil_id', $perfil_id);
                    $this->db->set('fk_permiso_id', $value);
                    $this->db->insert('perfil_permiso');
                }
            }
    ...
    public function editar($id='')
    {
        $this->db->trans_start();
            ...
            // Eliminar todos los perdiles = $id de la tabla perfil_permiso
            $this->db->where('fk_perfil_id', $id);
            $this->db->delete('perfil_permiso');        
            
            // Agregar la relacion perfil permiso
            $permisos = $this->input->post('permisos');
            if (isset($permisos)) {
                foreach ($permisos as $key => $value) {
                    $this->db->set('fk_perfil_id', $id);
                    $this->db->set('fk_permiso_id', $value);
                    $this->db->insert('perfil_permiso');
                }
            }
    ...
    public function eliminar()
    {
		...
		$this->db->trans_start();			
			...
			foreach ($ids as $key => $id) {
			    // Eliminar perfiles
			    $this->db->where('no_eliminar', 0);
				$this->db->where('perfil_id', $id);
	        	$this->db->delete('perfil');
                
                // Eliminar la relacion con el permiso de la tabla perfil - permiso
                $this->db->where('fk_perfil_id', $id);
                $this->db->delete('perfil_permiso');
			}
     ...
     * Perfil_permiso
     * 
     * Obtener la lista perfiles y permisos
     */
    
    public function get_perfil_permiso($id)
    {
        $this->db->where('fk_perfil_id', $id);
        return $this->db->get('perfil_permiso');
    }
}