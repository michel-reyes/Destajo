...
class Empresa extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->users->can('Empresa.Ver', TRUE);
	...
	public function index()
	{
        ...
        $this->load->view('empresa/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->empresa_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->empresa_m->count_all();
        $config['base_url'] = base_url('empresa/show_content');
        ...
        $response['registros'] = $this->load->view('empresa/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('empresa', 'Empresa', 'trim|required|xss_clean|max_length[150]|is_unique[empresa.empresa]');
    ...
    {  
        $this->users->can("Empresa.Agregar", TRUE);		
        $data['modulo'] = $this->modulo;
    ...
    {
        $this->users->can('Empresa.Editar', TRUE);        		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->empresa_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->empresa_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('Empresa.Eliminar', TRUE);		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->empresa_m->eliminar();
            if ( $query === TRUE) {
