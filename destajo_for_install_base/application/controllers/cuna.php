...
class Cuna extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->load->view('cuna/layout_v', $data);
	...
	{  
        ...
        // Obtener registros
        $results = $this->cuna_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->cuna_m->count_all();
        $config['base_url'] = base_url('cuna/show_content');
        ...
        $response['registros'] = $this->load->view('cuna/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('numero_operacional', 'N&uacute;mero operacional', 'trim|required|xss_clean|max_length[6]|is_unique[m_cuna.numero_operacional]');
    ...
    {
        $this->users->can('Cuna.Agregar', TRUE);
    ...
    {
        $this->users->can('Cuna.Editar', TRUE);
        		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->cuna_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->cuna_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('Cuna.Eliminar', TRUE);
        		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->cuna_m->eliminar();
            if ( $query === TRUE) {
