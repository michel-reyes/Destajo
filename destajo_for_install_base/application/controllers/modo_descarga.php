...
class Modo_descarga extends CI_Controller {
	
	protected $modulo = array(
        'nombre' => "modo_descarga", 
        'display' => "Modo de descarga"
    );
    ...
    {
        parent::__construct();
        $this->users->can('ModoDescarga.Ver', TRUE);
	...
	public function index()
	{
        ...
        $this->load->view('modo_descarga/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->modo_descarga_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->modo_descarga_m->count_all();
        $config['base_url'] = base_url('modo_descarga/show_content');
        ...
        $response['registros'] = $this->load->view('modo_descarga/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('modo', 'Modo de descarga', 'trim|required|xss_clean|max_length[200]|is_unique[m_modo_descarga.modo]');
    ...
    {
        $this->users->can('ModoDescarga.Agregar', TRUE);		
        $data['modulo'] = $this->modulo;
        ...
        // Mostrar formulario
         ...
         // Agregar
             ...
             $response = array();
             $query = $this->modo_descarga_m->agregar();
    ...
    {
        $this->users->can('ModoDescarga.Editar', TRUE);		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->modo_descarga_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->modo_descarga_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('ModoDescarga.Eliminar', TRUE);		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->modo_descarga_m->eliminar();
            if ( $query === TRUE) {
