<?php echo form_open('normativa/show_content', array('id'=>'form-search')); 
    // Obtener las busquedas almacenadas en session
    $b_campo = $this->session->userdata('buscar_campo_normativa');
    $b_criterio = $this->session->userdata('buscar_criterio_normativa');
    $b_texto = $this->session->userdata('buscar_texto_normativa');
?>
    			...
    			<tr>
    				...
    				<td>
                        <select class="select2" name="buscar_campo[]" data-placeholder="Columna">
                            <option value="normativa" <?php if($value=='normativa'): ?> selected="selected" <?php endif; ?>>Normativa</option>
