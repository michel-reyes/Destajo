<?php echo form_open('tiempo_carga/show_content', array('id'=>'form-search')); 
    // Obtener las busquedas almacenadas en session
    $b_campo = $this->session->userdata('buscar_campo_tiempo_carga');
    $b_criterio = $this->session->userdata('buscar_criterio_tiempo_carga');
    $b_texto = $this->session->userdata('buscar_texto_tiempo_carga');
?>
    			...
    			<tr>
    				...
    				<td>
                        <select class="select2" name="buscar_campo[]" data-placeholder="Columna">
                            ...
                            <option value="tiempo_carga" <?php if($value=='tiempo_carga'): ?> selected="selected" <?php endif; ?>>Tiempo de carga</option>
