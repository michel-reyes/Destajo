<?php if ($results->num_rows() > 0): ?>
	...
	<table class="table-fixed-header fancyTable">
		...
		<thead>
			<tr>				
				<th headers="checked"><input type="checkbox" />
					<?php echo anchor($modulo['nombre'] . "/show_content/?ofield=m_equipo.numero_operacional&otype=" . $otype, 'N&uacute;mero opercional'); ?>				
				</th>
			</tr>
		</thead>
		...
		<tbody>
			...
			<tr>
				<td headers="checked">
					<input type="checkbox" value="<?php echo $row->m_equipo_id; ?>" />
					<?php echo $row->numero_operacional; ?>
				</td>
			</tr>
			<?php endforeach; ?>
		</tbody>
