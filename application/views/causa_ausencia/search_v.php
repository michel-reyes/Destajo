<?php echo form_open('causa_ausencia/show_content', array('id'=>'form-search')); 
    // Obtener las busquedas almacenadas en session
    $b_campo = $this->session->userdata('buscar_campo_causa_ausencia');
    $b_criterio = $this->session->userdata('buscar_criterio_causa_ausencia');
    $b_texto = $this->session->userdata('buscar_texto_causa_ausencia');
?>
    			...
    			<tr>
    				...
    				<td>
                        <select class="select2" name="buscar_campo[]" data-placeholder="Columna">
                            <option value="causa" <?php if($value=='causa'): ?> selected="selected" <?php endif; ?>>Causa de la ausencia</option>
                        </select>
                    </td>
