<?php echo form_open('categoria_operario/show_content', array('id'=>'form-search')); 
    // Obtener las busquedas almacenadas en session
    $b_campo = $this->session->userdata('buscar_campo_categoria_operario');
    $b_criterio = $this->session->userdata('buscar_criterio_categoria_operario');
    $b_texto = $this->session->userdata('buscar_texto_categoria_operario');
?>
    			...
    			<tr>
    				...
    				<td>
                        <select class="select2" name="buscar_campo[]" data-placeholder="Columna">
                            ...
                            <option value="nomenclador" <?php if($value=='nomenclador'): ?> selected="selected" <?php endif; ?>>Nomenclador</option>
                            <option value="min_capacidad_carga" <?php if($value=='min_capacidad_carga'): ?> selected="selected" <?php endif; ?>>M&iacute;n. capacidad de carga</option>
                            <option value="max_capacidad_carga" <?php if($value=='max_capacidad_carga'): ?> selected="selected" <?php endif; ?>>M&aacute;x. capacidad de carga</option>
                        </select>
                    </td>
