...
class Claves_siscont_m extends CI_Model {
    ...
    {
        return $this->db->count_all_results('m_claves_siscont');
    }
	 ...
	 */
	 ...
	 public function list_all($limit='', $offset='')
	 {
         ...
         if (!$ofield OR !$otype) {
             $ofield = $this->session->userdata('ofield_claves_siscont') ? $this->session->userdata('ofield_claves_siscont') : "clave";
             $otype = $this->session->userdata('otype_claves_siscont') ? $this->session->userdata('otype_claves_siscont') : "asc";
         }
         ...
         $query['num_rows_wo_limit'] = $this->db->get('m_claves_siscont')->num_rows();
         if ($limit!='' OR $offset!='') $this->db->limit($limit, $offset);
        
         // Obtener registros
         //-----------------------------
         $query['rows'] = $this->db->get('m_claves_siscont');
         $query['num_rows'] = $query['rows']->num_rows();
         $this->db->flush_cache();
        
         // Almacenar ordenar en session
         //-----------------------------
         $this->session->set_userdata('order_field_claves_siscont', $ofield);
         $this->session->set_userdata('order_type_claves_siscont', $otype);
    ...
    public function create_search()
    {
        ...
        if (! $buscar_btn_avanzado) {
            if (! $campo OR count($campo) <= 0) {
                $campo = $this->session->userdata('buscar_campo_claves_siscont');
                $criterio = $this->session->userdata('buscar_criterio_claves_siscont');
                $texto = $this->session->userdata('buscar_texto_claves_siscont');
            }
        }
        ...
        if ($buscar_btn_avanzado AND $campo) {
            if ($this->input->post('buscar_campo')) $this->session->set_userdata('buscar_campo_claves_siscont', $this->input->post('buscar_campo'));
            if ($this->input->post('buscar_criterio')) $this->session->set_userdata('buscar_criterio_claves_siscont', $this->input->post('buscar_criterio'));
            if ($this->input->post('buscar_texto')) $this->session->set_userdata('buscar_texto_claves_siscont', $this->input->post('buscar_texto'));
        }
        
        // Se mando a buscar y no hay texto en la busqueda ni en session
        // so, eliminar de session
        if ($buscar_btn_avanzado AND ! $campo) {
            $this->session->unset_userdata('buscar_campo_claves_siscont');
            $this->session->unset_userdata('buscar_criterio_claves_siscont');
            $this->session->unset_userdata('buscar_texto_claves_siscont');
        }    
    ...
    public function get_by_id($id=0)
    {
        ...
        return $this->db->get('m_claves_siscont')->row();
    }
    ...
    public function agregar()
    {
        ...
        return $this->db->insert('m_claves_siscont');
    }
    ...
    public function eliminar()
    {
		...
		$this->db->trans_start();
			...
			foreach ($ids as $key => $id) {
				$this->db->where('m_claves_siscont_id', $id);
	        	$this->db->delete('m_claves_siscont');
			}
    ...
    public function get_all()
    {
        ...
        return $this->db->get('m_claves_siscont');
    }

    // ------------------------------------------------------------------------

    public function getClaveBySigla($sigla='')
    {
        $this->db->where('sigla', $sigla);
        return $this->db->get('m_claves_siscont', 1)->row()->valor;
    }
}