...
class Permiso_m extends CI_Model {
    ...
    {
        return $this->db->count_all_results('permiso');
    }
	 ...
	 */
	 ...
	 public function list_all($limit='', $offset='')
	 {
         ...
         if (!$ofield OR !$otype) {
             $ofield = $this->session->userdata('ofield_permiso') ? $this->session->userdata('ofield_permiso') : "nombre";
             $otype = $this->session->userdata('otype_permiso') ? $this->session->userdata('otype_permiso') : "asc";
         }
         ...
         $query['num_rows_wo_limit'] = $this->db->get('permiso')->num_rows();
         if ($limit!='' OR $offset!='') $this->db->limit($limit, $offset);
        
         // Obtener registros
         //-----------------------------
         $query['rows'] = $this->db->get('permiso');
         $query['num_rows'] = $query['rows']->num_rows();
         $this->db->flush_cache();
        
         // Almacenar ordenar en session
         //-----------------------------
         $this->session->set_userdata('order_field_permiso', $ofield);
         $this->session->set_userdata('order_type_permiso', $otype);
    ...
    public function create_search()
    {
        ...
        if (! $buscar_btn_avanzado) {
            if (! $campo OR count($campo) <= 0) {
                $campo = $this->session->userdata('buscar_campo_permiso');
                $criterio = $this->session->userdata('buscar_criterio_permiso');
                $texto = $this->session->userdata('buscar_texto_permiso');
            }
        }
        ...
        if ($buscar_btn_avanzado AND $campo) {
            if ($this->input->post('buscar_campo')) $this->session->set_userdata('buscar_campo_permiso', $this->input->post('buscar_campo'));
            if ($this->input->post('buscar_criterio')) $this->session->set_userdata('buscar_criterio_permiso', $this->input->post('buscar_criterio'));
            if ($this->input->post('buscar_texto')) $this->session->set_userdata('buscar_texto_permiso', $this->input->post('buscar_texto'));
        }
        
        // Se mando a buscar y no hay texto en la busqueda ni en session
        // so, eliminar de session
        if ($buscar_btn_avanzado AND ! $campo) {
            $this->session->unset_userdata('buscar_campo_permiso');
            $this->session->unset_userdata('buscar_criterio_permiso');
            $this->session->unset_userdata('buscar_texto_permiso');
        }    
