...
class Equipo_m extends CI_Model {
    ...
    {
        return $this->db->count_all_results('m_equipo');
    }
	 ...
	 */
	 ...
	 public function list_all($limit='', $offset='')
	 {
         ...
         if (!$ofield OR !$otype) {
             $ofield = $this->session->userdata('ofield_equipo') ? $this->session->userdata('ofield_equipo') : "numero_operacional";
             $otype = $this->session->userdata('otype_equipo') ? $this->session->userdata('otype_equipo') : "asc";
         }
         ...
         $query['num_rows_wo_limit'] = $this->db->get('m_equipo')->num_rows();
         if ($limit!='' OR $offset!='') $this->db->limit($limit, $offset);
        
         // Obtener registros
         //-----------------------------
         $query['rows'] = $this->db->get('m_equipo');
         $query['num_rows'] = $query['rows']->num_rows();
         $this->db->flush_cache();
        
         // Almacenar ordenar en session
         //-----------------------------
         $this->session->set_userdata('order_field_equipo', $ofield);
         $this->session->set_userdata('order_type_equipo', $otype);
    ...
    public function create_search()
    {
        ...
        if (! $buscar_btn_avanzado) {
            if (! $campo OR count($campo) <= 0) {
                $campo = $this->session->userdata('buscar_campo_equipo');
                $criterio = $this->session->userdata('buscar_criterio_equipo');
                $texto = $this->session->userdata('buscar_texto_equipo');
            }
        }
        ...
        if ($buscar_btn_avanzado AND $campo) {
            if ($this->input->post('buscar_campo')) $this->session->set_userdata('buscar_campo_equipo', $this->input->post('buscar_campo'));
            if ($this->input->post('buscar_criterio')) $this->session->set_userdata('buscar_criterio_equipo', $this->input->post('buscar_criterio'));
            if ($this->input->post('buscar_texto')) $this->session->set_userdata('buscar_texto_equipo', $this->input->post('buscar_texto'));
        }
        
        // Se mando a buscar y no hay texto en la busqueda ni en session
        // so, eliminar de session
        if ($buscar_btn_avanzado AND ! $campo) {
            $this->session->unset_userdata('buscar_campo_equipo');
            $this->session->unset_userdata('buscar_criterio_equipo');
            $this->session->unset_userdata('buscar_texto_equipo');
        }    
    ...
    {
        $this->db->limit(1);
        $this->db->where('m_equipo_id', $id);
        return $this->db->get('m_equipo')->row();
    }
    ...
    public function agregar()
    {
        ...
        return $this->db->insert('m_equipo');
    }
    ...
    public function editar($id='')
    {
        $this->db->set('numero_operacional', $this->input->post('numero_operacional'));
        $this->db->where('m_equipo_id', $id);
        return $this->db->update('m_equipo');
    }
