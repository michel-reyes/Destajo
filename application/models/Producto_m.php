...
class Producto_m extends CI_Model {
    ...
    {
        return $this->db->count_all_results('m_producto');
    }
	 ...
	 */
	 ...
	 public function list_all($limit='', $offset='')
	 {
         ...
         if (!$ofield OR !$otype) {
             $ofield = $this->session->userdata('ofield_producto') ? $this->session->userdata('ofield_producto') : "producto";
             $otype = $this->session->userdata('otype_producto') ? $this->session->userdata('otype_producto') : "asc";
         }
         ...
         $query['num_rows_wo_limit'] = $this->db->get('m_producto')->num_rows();
         if ($limit!='' OR $offset!='') $this->db->limit($limit, $offset);
        
         // Obtener registros
         //-----------------------------
         $query['rows'] = $this->db->get('m_producto');
         $query['num_rows'] = $query['rows']->num_rows();
         $this->db->flush_cache();
        
         // Almacenar ordenar en session
         //-----------------------------
         $this->session->set_userdata('order_field_producto', $ofield);
         $this->session->set_userdata('order_type_producto', $otype);
    ...
    public function create_search()
    {
        ...
        if (! $buscar_btn_avanzado) {
            if (! $campo OR count($campo) <= 0) {
                $campo = $this->session->userdata('buscar_campo_producto');
                $criterio = $this->session->userdata('buscar_criterio_producto');
                $texto = $this->session->userdata('buscar_texto_producto');
            }
        }
        ...
        if ($buscar_btn_avanzado AND $campo) {
            if ($this->input->post('buscar_campo')) $this->session->set_userdata('buscar_campo_producto', $this->input->post('buscar_campo'));
            if ($this->input->post('buscar_criterio')) $this->session->set_userdata('buscar_criterio_producto', $this->input->post('buscar_criterio'));
            if ($this->input->post('buscar_texto')) $this->session->set_userdata('buscar_texto_producto', $this->input->post('buscar_texto'));
        }
        
        // Se mando a buscar y no hay texto en la busqueda ni en session
        // so, eliminar de session
        if ($buscar_btn_avanzado AND ! $campo) {
            $this->session->unset_userdata('buscar_campo_producto');
            $this->session->unset_userdata('buscar_criterio_producto');
            $this->session->unset_userdata('buscar_texto_producto');
        }    
    ...
    public function get_by_id($id=0)
    {
        ...
        return $this->db->get('m_producto')->row();
    }
    ...
    public function agregar()
    {
        ...
        return $this->db->insert('m_producto');
    }
