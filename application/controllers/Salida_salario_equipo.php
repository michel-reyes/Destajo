...
class Salida_salario_equipo extends CI_Controller {
	
	protected $modulo = array(
        'nombre' => "salida_salario_equipo", 
        'display' => "Salario por equipo"
    );
    ...
    {
        parent::__construct();
        $this->users->can('SalidaSalarioEquipo.Ver', TRUE);
        ...
        $this->load->model('salida_salario_equipo_m');
	...
	public function index()
	{
        ...
        $this->load->view('salida_salario_equipo/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->salida_salario_equipo_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->salida_salario_equipo_m->count_all();
        $config['base_url'] = base_url('salida_salario_equipo/show_content');
        ...
        $response['registros'] = $this->load->view('salida_salario_equipo/tabla_v', $data, TRUE);
    ...
    */
    public function autoCalc()
    {
        ...
        $query = $this->salida_salario_equipo_m->autoCalc();
    ...
    {       
        ...
        if ($this->input->post('eliminar')) {
            $query = $this->salida_salario_equipo_m->eliminar();
            if ( $query === TRUE) {
    ...
    public function exportar()
    {
        ...
        $sql = read_file('./query_temp/sse_query_temp.tmp');
        ...
        // Name the worksheet
        $this->excel->getActiveSheet()->setTitle('Salida_salario_equipo');
        
        // Crear cabeceras
        $this->excel->getActiveSheet()->setCellValue('A1', 'Equipo');
        $this->excel->getActiveSheet()->setCellValue('B1', html_entity_decode('Cu&ntilde;a'));
        $this->excel->getActiveSheet()->setCellValue('C1', 'Importe del viaje');
        $this->excel->getActiveSheet()->setCellValue('D1', 'Inicio del periodo de pago');
        $this->excel->getActiveSheet()->setCellValue('E1', 'Final del periodo de pago');
        ...
        foreach ($query->result() as $row) {
            ...
            $this->excel->getActiveSheet()->setCellValue($map[$letra++] . "" . $j, $row->numero_operacional_equipo);
            $this->excel->getActiveSheet()->setCellValue($map[$letra++] . "" . $j, $row->numero_operacional_cuna);
        ...
        $date = date("d_m_y");
        $filename="Salida_salario_equipo_$date.xls";
    ...
    public function imprimir()
    {
        ...
        $this->load->view('template/t_header');
        $this->load->view('salida_salario_equipo/imprimir_v', $data);
        $this->load->view('template/t_footer');
