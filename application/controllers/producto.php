...
class Producto extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->users->can('Producto.Ver', TRUE);
	...
	public function index()
	{
        ...
        $this->load->view('producto/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->producto_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->producto_m->count_all();
        $config['base_url'] = base_url('producto/show_content');
        ...
        $response['registros'] = $this->load->view('producto/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('producto', 'Producto', 'trim|required|xss_clean|max_length[200]|is_unique[m_producto.producto]');
    ...
    {
        $this->users->can('Producto.Agregar', TRUE);		
        $data['modulo'] = $this->modulo;
    ...
    {
        $this->users->can('Producto.Editar', TRUE);		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->producto_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->producto_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('Producto.Eliminar', TRUE);		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->producto_m->eliminar();
            if ( $query === TRUE) {
