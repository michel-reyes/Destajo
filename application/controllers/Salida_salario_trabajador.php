...
class Salida_salario_trabajador extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        // Con las nuevas actualizaciones puede que un usuario desee
        // exportar a XML sin tener definido su fondo horario, en ese
        // caso poner el fondo horario cero (0)        
        if (isset($pp->fondo_horario) && empty($pp->fondo_horario))
        {
            $this->periodo_pago['fh'] == 0;
        }
        else
        {
    ...
    */
    ...
    public function exportar()
    {
		...
		$this->excel->getActiveSheet()->setCellValue('B1', 'CI');
        $this->excel->getActiveSheet()->setCellValue('C1', 'Nombre y apellidos');
        $this->excel->getActiveSheet()->setCellValue('D1', 'Importe del viaje');
		$this->excel->getActiveSheet()->setCellValue('E1', 'Importe del viaje minorista');
        $this->excel->getActiveSheet()->setCellValue('F1', 'Cumplimiento de la norma');
        $this->excel->getActiveSheet()->setCellValue('G1', 'Viaje');
		$this->excel->getActiveSheet()->setCellValue('H1', 'V.Minorista');
        $this->excel->getActiveSheet()->setCellValue('I1', 'Interrupto');
        $this->excel->getActiveSheet()->setCellValue('J1', 'No vinculado');
        $this->excel->getActiveSheet()->setCellValue('K1', 'Nocturnidad corta');
        $this->excel->getActiveSheet()->setCellValue('L1', 'Nocturnidad larga');
        $this->excel->getActiveSheet()->setCellValue('M1', html_entity_decode('Capacitaci&oacute;n'));
        $this->excel->getActiveSheet()->setCellValue('N1', 'Movilizado');
        $this->excel->getActiveSheet()->setCellValue('O1', 'Feriado');
        $this->excel->getActiveSheet()->setCellValue('P1', 'Ausencia');
        $this->excel->getActiveSheet()->setCellValue('Q1', 'Inicio del periodo de pago');
        $this->excel->getActiveSheet()->setCellValue('R1', 'Final del periodo de pago');
        ...
        foreach ($query->result() as $row) {
			...
			$this->excel->getActiveSheet()->setCellValue($map[$letra++] . "" . $j, $row->ci);
			...
			$this->excel->getActiveSheet()->setCellValue($map[$letra++] . "" . $j, mysql_to_number($row->importe_viaje_m));
			...
			$this->excel->getActiveSheet()->setCellValue($map[$letra++] . "" . $j, mysql_to_number($row->horas_viaje_m));
    ...
    public function xmlSiscont()
    {
        ...
        $cev = $this->claves_siscont_m->getClaveBySigla('CEV'); //2008

        // clave de entrada nocturnidad corta y larga
        $cenc = $this->claves_siscont_m->getClaveBySigla('CENC'); //2014
        ...
        foreach ($incidencia->result() as $i)
        {
            ...
            $tarifa_horario_escala = $this->math->L($this->math->S($i->chapa))->tarifa_horario_escala;

            // Rafael EXCEL to PHP
            //horas_viaje = si( horas_no_vinculado + horas_de_viaje > fondo horario; fondo_horario - horas_no_vinculado; horas_de_viaje )
            $hv = floatval($i->horas_viaje);
            $hvm = floatval($i->horas_viaje_m);
            $hnv = floatval($i->horas_no_vinculado);
            $horas_viaje = ( ($hv + $hvm +  $hnv) > $fondo_horario) ? ($fondo_horario - $hnv) : ($hv + $hvm);
            ...
            $xml .= $tab.$tab.$tab.'<TiemTarifa>'.$tarifa_horario_escala.'</TiemTarifa>'.$newline;
