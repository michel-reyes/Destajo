...
class Permiso extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->users->can('Permiso.Ver', TRUE);
	...
	public function index()
	{
        ...
        $this->load->view('permiso/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->permiso_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->permiso_m->count_all();
        $config['base_url'] = base_url('permiso/show_content');
        ...
        $response['registros'] = $this->load->view('permiso/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('nombre', 'Nombre', 'trim|required|max_length[150]|is_unique[permiso.nombre]');
        $this->form_validation->set_rules('descripcion', 'Descripci&oacute;n', 'trim|required|max_length[255]');
    ...
    {
        $this->users->can('Permiso.Agregar', TRUE);		
        $data['modulo'] = $this->modulo;
    ...
    {
        $this->users->can('Permiso.Editar', TRUE);		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->permiso_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->permiso_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('Permiso.Eliminar', TRUE);		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->permiso_m->eliminar();
            if ( $query === TRUE) {
