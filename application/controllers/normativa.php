...
date: 2014.03.10
type: php module
path: application/controllers/normativa.php
...
class Normativa extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->users->can('Normativa.Ver', TRUE);
        
        $this->load->model('normativa_m');
	...
	public function index()
	{
        ...
        $this->load->view('normativa/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->normativa_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->normativa_m->count_all();
        $config['base_url'] = base_url('normativa/show_content');
        ...
        $response['registros'] = $this->load->view('normativa/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('normativa', 'Normativa', 'trim|required|xss_clean|max_length[200]|is_unique[m_normativa.normativa]');
        $this->form_validation->set_rules('sigla', 'Sigla', 'trim|required|xss_clean|max_length[6]|is_unique[m_normativa.sigla]');
        $this->form_validation->set_rules('valor', 'Valor', 'trim|required|xss_clean|max_length[6]|decimal');
        $this->form_validation->set_rules('unidad_medida', 'Unidad de medida', 'trim|required|xss_clean|max_length[20]');
    ...
    {
        $this->users->can('Normativa.Agregar', TRUE);		
        $data['modulo'] = $this->modulo;
        ...
        // Mostrar formulario
         ...
         // Agregar
             ...
             $response = array();
             $query = $this->normativa_m->agregar();
    ...
    {
        $this->users->can('Normativa.Editar', TRUE);		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->normativa_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->normativa_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('Normativa.Eliminar', TRUE);		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->normativa_m->eliminar();
            if ( $query === TRUE) {
