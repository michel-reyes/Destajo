...
date: 2014.12.17
type: php module
path: application/controllers/claves_siscont.php

DESTAJO-MODULE-END
*/

class Claves_siscont extends CI_Controller {
	
	protected $modulo = array(
        'nombre' => "claves_siscont",
        'display' => "Claves siscont"
    );
    ...
    {
        parent::__construct();
        $this->users->can('ClavesSiscont.Ver', TRUE);
	...
	public function index()
	{
        ...
        $this->load->view('claves_siscont/layout_v', $data);
	...
	public function show_content($offset=0)
	{
        ...
        // Obtener registros
        $results = $this->claves_siscont_m->list_all($config['per_page'], $offset);
        ...
        $data['all_rows'] = $this->claves_siscont_m->count_all();
        $config['base_url'] = base_url('claves_siscont/show_content');
        ...
        $response['registros'] = $this->load->view('claves_siscont/tabla_v', $data, TRUE);
    ...
    public function validar()
    {
        ...
        $this->form_validation->set_rules('clave', 'Clave', 'trim|required|xss_clean|max_length[200]');
        $this->form_validation->set_rules('sigla', 'Sigla', 'trim|required|xss_clean|max_length[5]|is_unique[m_claves_siscont.sigla]');
        $this->form_validation->set_rules('valor', 'Valor', 'trim|required|xss_clean|max_length[10]');

        if ($this->input->post('unidad_medida'))
            $this->form_validation->set_rules('unidad_medida', 'Unidad de medida', 'trim|required|xss_clean|max_length[10]');    
    ...
    {  
        $this->users->can("ClavesSiscont.Agregar", TRUE);		
        $data['modulo'] = $this->modulo;
        ...
        // Mostrar formulario
         ...
         // Agregar
             ...
             $response = array();             
             $query = $this->claves_siscont_m->agregar();
    ...
    {
        $this->users->can('ClavesSiscont.Editar', TRUE);        		
        $data['modulo'] = $this->modulo;
        
        // Mostrar formulario
        if ( !$this->input->post('accion') || $this->input->post('accion') != "editar" ){
            $data['ibi'] = $this->claves_siscont_m->get_by_id($id);
            $this->load->view($this->modulo['nombre'] . '/editar_v', $data);
        }
        // Editar
        else {
            $response = array();
            $query = $this->claves_siscont_m->editar( $this->input->post('id') );
    ...
    {
        $this->users->can('ClavesSiscont.Eliminar', TRUE);		
        $response = array();
        
        if ($this->input->post('eliminar')) {
            $query = $this->claves_siscont_m->eliminar();
            if ( $query === TRUE) {
