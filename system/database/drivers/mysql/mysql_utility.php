...
class CI_DB_mysql_utility extends CI_DB_utility {
	...
	function _backup($params = array())
	{
		...
		foreach ((array)$tables as $table)
		{
			...
			$j = 0;
            $jcount = count($query->result_array());
			foreach ($query->result_array() as $row)
			{
				...
				// Build the INSERT string
                // Minimizar el fichero de salida con values aninados por cada insert
                if ($j == 0) {
                    $output .= 'INSERT INTO '.$table.' ('.$field_str.') VALUES' . $newline . '('.$val_str.')';
                }else
                if ($j > 0) {
                    $output .= '('.$val_str.')';
                } 
